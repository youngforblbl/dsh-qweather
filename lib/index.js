...
function tempChartSvg(hours) {
	...
	const step = 364 / Math.max(1, hours.length - 1);
	const points = hours.map((hour, index) => {
		return [r1(CHART_PAD_X + index * step), r1(CHART_BOTTOM - (hour.temp - min) / span * 46)];
	});
	const line = smoothPath(points);
	const last = points[points.length - 1];
	const first = points[0];
	const area = line + " L" + last[0] + ",80 L" + first[0] + ",80 Z";
	...
	return `<svg class="qw-chart" viewBox="0 0 ${CHART_W} ${CHART_H}" preserveAspectRatio="none" aria-label="气温曲线"><defs><linearGradient id="qw-chart-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="var(--ac)" stop-opacity="0.28"/><stop offset="100%" stop-color="var(--ac)" stop-opacity="0"/></linearGradient></defs><path class="qw-chart-area" d="${area}" fill="url(#qw-chart-fill)"/><path class="qw-chart-line" d="${line}"/>` + dots + labels + `</svg>`;
}
