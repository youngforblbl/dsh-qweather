...
function tempChartSvg(hours) {
	...
	return `<div class="qw-chart">${`<svg class="qw-chart-svg" viewBox="0 0 ${CHART_W} ${CHART_H}" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="qw-chart-stroke" gradientUnits="userSpaceOnUse" x1="0" y1="34" x2="0" y2="84"><stop offset="0%" style="stop-color:var(--orange)"/><stop offset="100%" style="stop-color:var(--sky)"/></linearGradient></defs><path class="qw-chart-line" d="${line}" stroke="url(#qw-chart-stroke)"/></svg>`}${hours.map((hour, index) => {
		const [x, y] = points[index];
		return `<span class="qw-chart-chip" style="left:${10 + index * 20}%;top:calc(${r1(y / CHART_H * 100)}% - 9px)">${escapeHtml(round1(hour.temp))}℃</span>`;
	}).join("")}</div>`;
}
...
async function readJsonBody(request) {
	...
	for await (const chunk of request) {
		...
		size += buffer.length;
		if (size > 16384) throw new Error("请求体过大");
		chunks.push(buffer);
	}
...
function mountConfigRoutes(webServer, deps) {
		...
		handler: async (request, response) => {
			...
			if (request.method === "POST") {
				...
				try {
					...
					sendJson(response, 200, { config: await deps.updateConfig(patch) });
				} catch (error) {
	...
	ctx.inject(["webServer"], (wctx) => {
		...
		wctx.effect(() => mountConfigRoutes(webServer, {
			...
			updateConfig: async (patch) => {
				if (scope === void 0) throw new Error("设置服务不可用，无法保存配置");
				Config(patch);
