...
function tempChartSvg(hours) {
	...
	const area = line + " L" + last[0] + ",92 L" + first[0] + ",92 Z";
	return `<div class="qw-chart">${`<svg class="qw-chart-svg" viewBox="0 0 ${CHART_W} ${CHART_H}" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="qw-chart-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" style="stop-color:var(--sky)" stop-opacity="0.30"/><stop offset="70%" style="stop-color:var(--sky)" stop-opacity="0.06"/><stop offset="100%" style="stop-color:var(--sky)" stop-opacity="0"/></linearGradient><linearGradient id="qw-chart-stroke" gradientUnits="userSpaceOnUse" x1="0" y1="0" x2="400" y2="0"><stop offset="0%" style="stop-color:var(--sky)"/><stop offset="100%" style="stop-color:var(--orange)"/></linearGradient><filter id="qw-chart-glow" x="-20%" y="-30%" width="140%" height="160%"><feGaussianBlur stdDeviation="2.6" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs><path d="${area}" fill="url(#qw-chart-fill)"/><path class="qw-chart-shadow" d="${line}" transform="translate(0,1.8)" style="stroke:light-dark(rgba(148,163,184,.55),rgba(0,0,0,.5))"/><path class="qw-chart-line" d="${line}" stroke="url(#qw-chart-stroke)" filter="url(#qw-chart-glow)"/><path class="qw-chart-ridge" d="${line}" transform="translate(0,-1.1)" style="stroke:light-dark(rgba(255,255,255,.95),rgba(255,255,255,.25))"/></svg>`}${hours.map((hour, index) => {
		const [x, y] = points[index];
		const left = 10 + index * 20;
		const top = r1(y / CHART_H * 100);
		return `<span class="qw-chart-chip" style="left:${left}%;top:calc(${top}% - 7px)">${escapeHtml(round1(hour.temp))}℃</span><span class="qw-chart-dot" style="left:${left}%;top:${top}%"></span>`;
	}).join("")}</div>`;
}
