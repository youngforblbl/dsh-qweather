...
function tempChartSvg(hours) {
	...
	const step = 360 / Math.max(1, hours.length - 1);
	const points = hours.map((hour, index) => {
		return [r1(CHART_PAD_X + index * step), r1(CHART_BOTTOM - (hour.temp - min) / span * 43)];
	});
	const line = smoothPath(points);
	const last = points[points.length - 1];
	const first = points[0];
	const area = line + " L" + last[0] + ",84 L" + first[0] + ",84 Z";
	...
	return `<svg class="qw-chart" viewBox="0 0 ${CHART_W} ${CHART_H}" preserveAspectRatio="none" aria-label="气温曲线"><defs><linearGradient id="qw-chart-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" style="stop-color:var(--sky)" stop-opacity="0.32"/><stop offset="70%" style="stop-color:var(--sky)" stop-opacity="0.08"/><stop offset="100%" style="stop-color:var(--sky)" stop-opacity="0"/></linearGradient><filter id="qw-chart-glow" x="-20%" y="-30%" width="140%" height="160%"><feGaussianBlur stdDeviation="2.6" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs><path class="qw-chart-area" d="${area}" fill="url(#qw-chart-fill)"/><path class="qw-chart-line" d="${line}" filter="url(#qw-chart-glow)"/>` + dots + labels + `</svg>`;
}
