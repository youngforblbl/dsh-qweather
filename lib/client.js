	...
	factory: (require) => {
		...
		*/
		...
		function drop(cx, cy, s, color) {
			return `<path transform="translate(${cx - 12.4},${cy - 15.2}) scale(${s})" d="M12.4 12.2c0 2.5 1.5 4.1 3 4.1s3-1.6 3-4.1c0-2.2-3-4.7-3-4.7s-3 2.5-3 4.7z" fill="${color}"/><circle cx="${cx - .7}" cy="${cy + .2}" r="${.7 * s}" fill="#ffffff" opacity=".75"/>`;
		}
		/** 雪花：三条圆头短线 + 中心点。 */
		function flake(cx, cy, s) {
			return `<g transform="translate(${cx - 12},${cy - 12}) scale(${s})" stroke="#ffffff" stroke-width="2" stroke-linecap="round"><path d="M12 7.6v8.8M8.2 9.8l7.6 4.4M15.8 9.8l-7.6 4.4"/></g><circle cx="${cx}" cy="${cy}" r="${1.1 * s}" fill="#ffffff"/>`;
		}
		/** 太阳本体 + 光芒（fill/line 双色渐变）。 */
		function sun(uid, cx, cy, s, rays) {
			const c = U(uid);
			return (rays ? `<g transform="translate(${cx - 12},${cy - 12}) scale(${s})" stroke="${c}" stroke-width="2.3" stroke-linecap="round"><path d="M12 2.4v2.4M12 19.2v2.4M2.4 12h2.4M19.2 12h2.4M5.2 5.2l1.7 1.7M17.1 17.1l1.7 1.7M18.8 5.2l-1.7 1.7M6.9 17.1l-1.7 1.7"/></g>` : "") + `<circle cx="${cx}" cy="${cy}" r="${4.5 * s}" fill="${c}"/><circle cx="${cx - 1.2 * s}" cy="${cy - 1.2 * s}" r="${1.8 * s}" fill="#ffffff" opacity=".4"/>`;
		}
		/** 月牙 + 星光。 */
		function moon(uid, cx, cy, s) {
			const c = U(uid);
			return `<path transform="translate(${cx - 12},${cy - 12}) scale(${s})" d="M19.6 12.8A8.6 8.6 0 1 1 11.2 3.4a7.2 7.2 0 0 0 8.4 9.4z" fill="${c}"/><circle cx="${cx - 3.2 * s}" cy="${cy - 4 * s}" r="${1 * s}" fill="#ffffff" opacity=".85"/><circle cx="${cx - 1.4 * s}" cy="${cy - 1 * s}" r="${.6 * s}" fill="#ffffff" opacity=".6"/>`;
		}
		...
		function weatherIcon(code, size = 24, uid = "ic") {
			...
			const body = (BODIES[kind] ?? BODIES["unknown"])(uid, a, b);
		...
		const BODIES = {
			...
			rain: (uid, a, b) => {
				...
				return cloudShapes(uid, c) + cloudShade() + `<defs><linearGradient id="qw-ic-${uid}-w" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" style="stop-color:${WATER_A}"/><stop offset="100%" style="stop-color:${WATER_B}"/></linearGradient></defs>` + drop(10, 15.6, 1, w) + drop(14.6, 15.6, 1, w);
			},
			"heavy-rain": (uid, a, b) => {
				const c = U(uid);
				const w = `url(#qw-ic-${uid}-w)`;
				return cloudShapes(uid, c) + cloudShade() + `<defs><linearGradient id="qw-ic-${uid}-w" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" style="stop-color:${WATER_A}"/><stop offset="100%" style="stop-color:${WATER_B}"/></linearGradient></defs>` + drop(8.6, 15.6, 1, w) + drop(12.4, 16, 1, w) + drop(16.2, 15.6, 1, w);
			},
			thunder: (uid, a, b) => {
				const c = U(uid);
				const bolt = `url(#qw-ic-${uid}-bolt)`;
				return `<g transform="translate(0,-1.2)">${cloudShapes(uid, c)}${cloudShade()}</g><defs><linearGradient id="qw-ic-${uid}-bolt" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" style="stop-color:${BOLT_A}"/><stop offset="100%" style="stop-color:${BOLT_B}"/></linearGradient></defs><path d="M13.6 12.6l3.6-4.4h-2.4l1-3.2-3.9 4.6h2.5z" fill="${bolt}" stroke="${bolt}" stroke-width="1.2" stroke-linejoin="round"/><path d="M13.9 9.4l1.2-1.4" stroke="#ffffff" stroke-width="1" stroke-linecap="round" opacity=".8"/>`;
			},
			sleet: (uid, a, b) => {
				const c = U(uid);
				const w = `url(#qw-ic-${uid}-w)`;
				return cloudShapes(uid, c) + cloudShade() + `<defs><linearGradient id="qw-ic-${uid}-w" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" style="stop-color:${WATER_A}"/><stop offset="100%" style="stop-color:${WATER_B}"/></linearGradient></defs>` + drop(9.6, 15.4, .9, w) + flake(15.4, 16.4, .9);
			},
			snow: (uid) => {
				return cloudShapes(uid, U(uid)) + cloudShade() + flake(8.4, 16, .92) + flake(12.4, 16.8, .92) + flake(16.4, 16, .92);
			},
			fog: (uid, a, b) => {
				const c = U(uid);
				const l = `url(#qw-ic-${uid}-f)`;
				return `<g transform="translate(0,-1.6)">${cloudShapes(uid, c)}${cloudShade()}</g><defs><linearGradient id="qw-ic-${uid}-f" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" style="stop-color:${a}"/><stop offset="100%" style="stop-color:${b}"/></linearGradient></defs><g stroke="${l}" stroke-width="2.4" stroke-linecap="round"><path d="M6 16.6h12M8 19.4h8M10 22.2h4"/></g>`;
			},
			haze: (uid, a, b) => {
				return `<defs><linearGradient id="qw-ic-${uid}-f" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" style="stop-color:${a}"/><stop offset="100%" style="stop-color:${b}"/></linearGradient></defs><g stroke="${`url(#qw-ic-${uid}-f)`}" stroke-width="2.6" stroke-linecap="round"><path d="M4.5 8h15M6.5 12h11M4.5 16h15M8 20h8"/></g><circle cx="16.5" cy="5.6" r="1" fill="#cbd7e8" opacity=".7"/>`;
			},
			unknown: (uid) => {
		...
		*/
		...
		const card = {
			...
			border: "1px solid light-dark(rgba(255,255,255,.75),rgba(255,255,255,.09))",
			borderRadius: 16,
			background: "radial-gradient(130% 100% at 10% -10%,light-dark(rgba(56,189,248,.20),rgba(76,141,255,.16)),transparent 46%),radial-gradient(120% 110% at 105% 108%,light-dark(rgba(249,115,22,.12),rgba(251,146,60,.10)),transparent 48%),linear-gradient(150deg,light-dark(rgba(255,255,255,.88),rgba(34,47,70,.80)),light-dark(rgba(255,255,255,.55),rgba(16,24,40,.74)))",
			backdropFilter: "blur(14px) saturate(1.15)",
			WebkitBackdropFilter: "blur(14px) saturate(1.15)",
			boxShadow: "0 12px 28px light-dark(rgba(100,116,139,.20),rgba(0,0,0,.45)),8px 8px 20px light-dark(rgba(148,163,184,.42),rgba(0,0,0,.55)),-8px -8px 20px light-dark(rgba(255,255,255,.95),rgba(90,110,140,.14)),inset 0 1px 0 light-dark(rgba(255,255,255,.9),rgba(255,255,255,.07))"
		};
		const row = {
			display: "flex",
			alignItems: "center",
		...
		const hourCell = {
			...
			borderRadius: 11,
			border: "1px solid light-dark(rgba(255,255,255,.75),rgba(255,255,255,.09))",
			background: "linear-gradient(145deg,light-dark(rgba(255,255,255,.85),rgba(38,51,73,.75)),light-dark(rgba(233,239,247,.75),rgba(20,29,45,.70)))",
			boxShadow: "inset 2px 2px 5px light-dark(rgba(148,163,184,.42),rgba(0,0,0,.55)),inset -2px -2px 5px light-dark(rgba(255,255,255,.95),rgba(90,110,140,.14))"
		};
		const tile = {
			flex: "none",
			display: "inline-flex",
			...
			boxShadow: "4px 4px 10px light-dark(rgba(148,163,184,.42),rgba(0,0,0,.55)),-3px -3px 8px light-dark(rgba(255,255,255,.95),rgba(90,110,140,.14)),inset 0 1px 0 light-dark(rgba(255,255,255,.95),rgba(255,255,255,.09))"
		};
		function Icon({ code, size, uid }) {
		...
		/** 新拟态图标块。 */
		function IconTile({ code, size, tileSize, uid }) {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				style: {
					...tile,
					width: tileSize,
					height: tileSize,
					borderRadius: Math.round(tileSize * .3)
				},
				dangerouslySetInnerHTML: { __html: weatherIcon(code, size, uid) }
			});
		}
		/** 迷你气温曲线（渐变面积 + 平滑折线 + 描点，随侧边栏宽度自适应）。 */
		function MiniCurve({ hours }) {
			...
			const ys = temps.map((temp) => 39 - (temp - min) / span * 30);
			const points = xs.map((x, index) => x.toFixed(1) + "," + ys[index].toFixed(1)).join(" ");
			const area = "M" + xs[0].toFixed(1) + ",46 L" + points + " L" + xs[xs.length - 1].toFixed(1) + ",46 Z";
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("svg", {
				...
				children: [
					...
					xs.map((x, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("circle", {
						cx: x,
						cy: ys[index],
						r: 2.4,
						style: {
							fill: "light-dark(#ffffff,#101a2e)",
							stroke: sky,
							strokeWidth: 1.8
						}
					}, index))
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: alerts.map((alert) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					style: {
						...
						boxShadow: "1px 2px 6px light-dark(rgba(148,163,184,.42),rgba(0,0,0,.55))",
						overflow: "hidden",
		...
		function RailView({ bundle, status, error, onExpand }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
				...
				children: [now !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					style: {
						...tile,
						width: 30,
						height: 30,
					...
					dangerouslySetInnerHTML: { __html: weatherIcon(now.icon, 18, "rail") }
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					now !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						style: row,
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconTile, {
								code: now.icon,
								size: 24,
								tileSize: 38,
								uid: "now"
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: round1(now.temp) }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									style: {
										fontSize: 15,
										color: orange
									},
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								...
								children: [now.feelsLike !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: ["体感 ", /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("b", {
									style: {
										color: fg,
										fontWeight: 700,
										...num
									},
									children: [round1(now.feelsLike), "°"]
								})] }), now.humidity !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: ["湿度 ", /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("b", {
									style: {
										color: fg,
										fontWeight: 700,
										...num
					...
					hours.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							style: {
								display: "flex",
								alignItems: "center",
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { style: {
								...
								height: 11,
								borderRadius: 2,
								background: "linear-gradient(180deg,light-dark(#38bdf8,#4c8dff),light-dark(#f97316,#fb923c))"
							} }), t("widget.hourly")]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							style: hourGrid,
							children: hours.map((hour, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								style: hourCell,
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										style: {
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(MiniCurve, { hours })
					] }),
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						style: {
							...
							fontSize: 10,
							color: faint,
							borderTop: "1px dashed light-dark(rgba(255,255,255,.75),rgba(255,255,255,.09))",
							paddingTop: 8
						},
