...
function clientFrom(config: QWeatherRuntimeConfig, signal: AbortSignal | undefined): QWeatherClient {
  if (!config.enabled) {
    throw new Error('和风天气插件已在设置中被关闭：请到 设置 → 插件 → 和风天气 打开总开关')
  }
  const apiKey = config.apiKey?.trim() ?? ''
  if (apiKey.length === 0) {
    throw new Error('尚未配置和风天气 API KEY：请到 设置 → 插件 → 和风天气 填写密钥')
  }
  return new QWeatherClient({ apiHost: config.apiHost, apiKey, signal })
}
 ...
 */
export function weatherTool(ctx: Context, getConfig: () => QWeatherRuntimeConfig): ToolDefinition {
  return defineTool({
 ...
 */
export function cardTool(ctx: Context, getConfig: () => QWeatherRuntimeConfig): ToolDefinition {
  return defineTool({
    name: CARD_TOOL_NAME,
    description:
      '在对话中渲染一张交互式天气卡片（HTML），方便用户浏览阅读。'
      + ' location 可缺省（默认用设置里配置的位置；支持城市/区县名称、LocationID、或“经度,纬度”）。'
      + ' hours 为卡片显示的逐小时预报条数（1-24，默认 5）。'
      + ' 卡片包含：当前天气（图标+文字+气温）、未来 N 小时天气/降水概率/气温曲线、黄色及以上重要预警、信息更新时间。'
