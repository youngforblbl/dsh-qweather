...
DeepSeek Harness 的和风天气（QWeather）插件：把实时天气带进侧边栏、带给 LLM、画进对话流。

[English](README.en.md) · [UI 预览](preview.html) · [API 文档](https://dev.qweather.com/docs/api/)
...
| 2 | **侧边栏天气组件** | 侧边栏底部（`sidebar.footer.action`）：展开显示当前天气（图标+文字）、气温、未来 5 小时天气/降水概率/气温曲线（缩略）、黄色以上预警、更新时间；收起只显示天气图标 + 气温，点击展开侧边栏 |
| 3 | **LLM 天气接口** | 工具 `qweather_weather(location?, range?, hours?, days?, fields?)`：按“位置 + 时间区间 + 关心的信息”自动选择对应 API 取数，返回结构化摘要给模型作答 |
| 4 | **对话内天气卡片** | 工具 `qweather_card(location?, hours?)`：把当前天气、未来 N 小时 + 气温曲线、黄色以上预警、更新时间画成交互式 HTML 卡片直接渲染进会话流（可回放） |
...
dsh plugin --profile web add github:<你的用户名>/dsh-qweather
...
```bash
dsh plugin --profile desktop add github:<你的用户名>/dsh-qweather
# 重启 DSH Desktop 后生效
```
...
展开侧边栏即可看到底部天气卡片：地点、当前天气（图标+文字+气温+体感/湿度）、未来 5 小时（时间/图标/降水概率/气温）、黄色及以上预警（最多 2 条）、更新时间；右上角 ↻ 手动刷新，数据每 10 分钟自动刷新。收起侧边栏只剩图标 + 气温，点击即可展开。
...
把杭州未来 3 天天气画成卡片给我看                    # 模型会调 qweather_card
...
- `qweather_card`：`location`（缺省用设置位置）、`hours=1-24`（卡片显示的小时数，默认 5）。

### 卡片内容

当前天气（图标+文字+气温）→ 未来 N 小时逐时天气/降水概率 + 气温曲线（内联 SVG）→ 黄色及以上预警（蓝/未知级别按“非重要预警”过滤）→ 信息更新时间 + 数据来源。卡片随 DSH 明暗主题切换，会话重放时逐字节还原，不依赖网络。
...
├─ tools.ts             工具定义（参数 schema / 输出投影 / 并发安全标记）
...
│  └─ card.ts           数据 → 卡片 HTML fragment（含气温曲线 SVG）
└─ client/              浏览器半端（lib/client.js，/plugins/dsh-qweather/client.js）
   ...
   ├─ use-qweather.ts    scope 订阅 + 自动/手动定位 + 定时刷新 hook
...
- **设置驱动**：主机 `installSettingsSection('qweather', Config)` 注册命名空间，客户端 `settingsScope.bind('qweather')` 读写；用户改动即时生效，无需重启。
...
- API KEY 以普通字段保存在本机 `~/.dsh/settings.yaml`（便于设置卡回显）；如需更严格管理，见下文升级路线（credentials 域 + `role('secret')`）。
- 插件不向任何第三方上报数据；天气请求只发往你配置的 API Host。

## 开发

要求 Node ≥ 22（本仓库在 Node 24 / pnpm 11 上验证）。
...
工程约定：`lib/` 构建产物**随仓库提交**（`dsh plugin add` 直接可用）；`src/qweather/` 保持零依赖纯函数（新增功能先写测试再写实现）；客户端组件不 import 主机包（槽位键/工具名统一放在 `types.ts`）。

## 发布到 GitHub / awesome-dsh-plugin

1. 推送到 GitHub，确保 `lib/`、`cordis.patch.yml`、`package.json#files` 已就位；
2. 按 [awesome-dsh-plugin](https://github.com/awesome-dsh-plugin/awesome-dsh-plugin) 的 README 提交条目（附仓库链接与中文简介）；
3. 站点与 dshmarket 插件市场会自动收录（通常一天内生效），用户即可 `dsh plugin --profile web add github:…` 安装。
